...
转自：https://www.cnblogs.com/hejiale010426/archive/2023/06/05/17457761.html
...
执行流程如下：
创建一个聚合对象，并添加一些元素。
通过聚合对象的 `CreateIterator` 方法创建一个迭代器。
通过迭代器的 `First` 方法获取第一个元素，然后通过 `Next` 方法获取后续的元素，直到获取不到元素为止。
